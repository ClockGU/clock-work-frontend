...
class EmployeeData {
  ...
  constructor(data = {}) {
    ...
    this.date_of_birth = data.date_of_birth ? new Date(data.date_of_birth) : null;
  ...
  static fromBackendResponse(data) {
    ...
    if (typeof modelData.prev_emp_duration === 'string' && modelData.prev_emp_duration.includes(' – ')) {
  ...
  toBackendFormat() {
    ...
    if (
      ...
      const endDate = formattedData.prev_emp_duration[formattedData.prev_emp_duration.length - 1];
